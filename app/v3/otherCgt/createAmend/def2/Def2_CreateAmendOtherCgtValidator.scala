...
class Def2_CreateAmendOtherCgtValidator(nino: String, taxYear: String, body: JsValue) extends Validator[CreateAmendOtherCgtRequestData] {

  private lazy val resolveJson = new ResolveNonEmptyJsonObject[request.Def2_CreateAmendOtherCgtRequestBody]()

  def validate: Validated[Seq[MtdError], Def2_CreateAmendOtherCgtRequestData] =
    (
      ResolveNino(nino),
      resolveJson(body)
    ).mapN { (validNino, requestBody) =>
      Def2_CreateAmendOtherCgtRequestData(
        nino = validNino,

...
@Singleton
class Def3_RetrieveCgtResidentialPropertyValidator @Inject() (nino: String, taxYear: String)
  ...
  override def validate: Validated[Seq[MtdError], RetrieveCgtResidentialPropertyRequestData] =
    ResolveNino(nino).map(validNino => Def3_RetrieveResidentialPropertyRequestData(validNino, TaxYear.fromMtd(taxYear)))

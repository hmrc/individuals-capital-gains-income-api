...
case class ResolveAssetType(error: MtdError) extends ResolverSupport {
  ...
  def isValid(assetType: String): Boolean = assetType != null &&
    (assetType.matches("other-property") ||
      assetType.matches("unlisted-shares") ||
      assetType.matches("listed-shares") ||
      assetType.matches("other-asset"))
}

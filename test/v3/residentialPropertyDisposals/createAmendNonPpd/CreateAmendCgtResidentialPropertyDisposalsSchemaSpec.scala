...
import v3.residentialPropertyDisposals.createAmendNonPpd.CreateAmendCgtResidentialPropertyDisposalsSchema.*

class CreateAmendCgtResidentialPropertyDisposalsSchemaSpec
    extends UnitSpec
    with MockAppConfig
  ...
  "schema lookup" when {
    "a valid tax year is supplied" must {
      "use Def1 schema for tax years between 2020-21 and 2024-25" in new Test {
    ...
    "handle errors" when {
      "an invalid tax year is supplied" must {
        "disallow tax years prior to 2020-21 and return RuleTaxYearNotSupportedError" in new Test {
          forTaxYearsBefore(TaxYear.fromMtd("2020-21")) { taxYear =>

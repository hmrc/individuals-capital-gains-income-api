...
class Def3_CreateAmendCgtResidentialPropertyDisposalsRequestBodySpec extends UnitSpec {
  ...
  private val validModel = Def3_CreateAmendCgtResidentialPropertyDisposalsRequestBody(disposals = Seq(
    Disposal(
      numberOfDisposals = 3,
  ...
  private val emptyModel = Def3_CreateAmendCgtResidentialPropertyDisposalsRequestBody(disposals = Seq.empty)

  "reads" should {
    "read to a case class" when {
      "provided valid JSON" in {
        validMtdJson.as[Def3_CreateAmendCgtResidentialPropertyDisposalsRequestBody] shouldBe validModel
      }
    }
  }
